...
from model.data.detection_utils import build_strong_augmentation


class DatasetMapperWithAugmentation(DatasetMapper):
    """
    ...
    def __init__(self, cfg, is_train=True):
        ...
        self.num_aug = cfg.SIS.NUM_AUGMENTATION
        # print("printing num of augmentation : ",cfg.SIS.NUM_AUGMENTATION)
        # sys.exit(-1)
    ...
    def __call__(self, dataset_dict):
        ...
        ret_dataset_dicts = [dataset_dict]

        for itr in range(4):

          augApply = build_strong_augmentation(self.is_train, itr)

          image_pil = Image.fromarray(image_weak_aug.astype("uint8"), "RGB")
          image_strong_aug = np.array(augApply(image_pil))
          dataset_dict_aug = copy.deepcopy(dataset_dict)
          dataset_dict_aug["image"] = torch.as_tensor(
              np.ascontiguousarray(image_strong_aug.transpose(2, 0, 1))
          )
          # print("Printing height/width of weak image : ",dataset_dict_aug["image"].size(1))
          # sys.exit(-1)
          assert dataset_dict["image"].size(1) == dataset_dict_aug["image"].size(1)
          assert dataset_dict["image"].size(2) == dataset_dict_aug["image"].size(2)
          ret_dataset_dicts.append(dataset_dict_aug)
        
        # print("Inside dataset mapper")
        # print("dataset mapper returning tuple with size : ",len(tuple(ret_dataset_dicts)))
        # sys.exit(-1)

        return tuple(ret_dataset_dicts)