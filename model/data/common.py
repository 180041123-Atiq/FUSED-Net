...
class AspectRatioGroupedDatasetTwoCrop(AspectRatioGroupedDataset):
    ...
    def __iter__(self):
        for d in self.dataset:
            ...
            # print("Inside ModelDataCommon AspectTwoCrop")
            # print("Dataset Element Type : ",type(d))
            # print("dataset Element Length : ",len(d))
            # sys.exit(-1)

            self.bucket[0].append(d[0])
            self.bucket[1].append(d[1])
            self.bucket[2].append(d[2])
            self.bucket[3].append(d[3])
            self.bucket[4].append(d[4])

            if len(self.bucket[0]) == self.batch_size:
                yield (self.bucket[0][:],self.bucket[1][:],self.bucket[2][:],self.bucket[3][:],self.bucket[4][:])
                del self.bucket[0][:]
                del self.bucket[1][:]
                del self.bucket[2][:]
                del self.bucket[3][:]
            ...
            # # d is a tuple with len = 5
            # # It's five images (same size) from the same image instance
